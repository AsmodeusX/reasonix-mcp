...
| `reasonix_broadcast(session_ids, message, expect?, parallelism?)` | Send one instruction to selected agents concurrently. Returns compact per-agent delivery/failure results; successful sends are not rolled back if another agent fails. |
...
| `reasonix_list(include_task?, pending_only?, detail?)` | Compact fleet inventory with aggregate status counts. `pending_only=true` returns only active, decision-blocked, or uncollected terminal sessions. `detail=true` opts into fleet-wide plans/config/transcript paths; prefer `poll` for one agent. |
...
another local service. Credentials are never included in the URL. The browser
retains an accepted password only in tab-scoped session storage; runtime state
is stored mode `0600` in
...
untouched. The replacement preserves port 8746 and its configured password,
