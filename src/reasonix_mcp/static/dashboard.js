const state = { password: "", fleet: null, routines: [], templates: [], selected: null, selectedRoutine: null, detail: null, refreshTimer: null, refreshSessions: new Set(), refreshAll: false, refreshing: false, collapsedOwners: new Set(), originalConfig: {}, timelineSession: null, timelineSignature: "" };
const $ = id => document.getElementById(id);

function savedPassword() { return sessionStorage.getItem("reasonix-dashboard-password") || ""; }
function routeFromLocation() { const p=new URLSearchParams(location.hash.replace(/^#/,""));return {session:p.get("session")||"",routine:p.get("routine")||""}; }
function setRoute(kind="",id="") { const url=new URL(location.href);url.hash=kind&&id?new URLSearchParams({[kind]:id}).toString():"";history.replaceState(null,"",url); }
...
async function refreshFleet(keepSelection=true) {
  const [fleet,routineData,templateData] = await Promise.all([api("/api/fleet"),api("/api/routines"),api("/api/routine-templates")]);
  state.fleet=fleet;state.routines=routineData.routines||[];state.templates=templateData.templates||[];
  renderFleet();renderRoutines();
  if (keepSelection && state.selected) {
    const found=selectedSummary();
    if(found) renderSummary(found);
  }
  if(keepSelection&&state.selectedRoutine){if(selectedRoutine())renderRoutineDetail();else{$("routine-view").classList.add("hidden");$("empty").classList.remove("hidden");state.selectedRoutine=null;setRoute();}}
...
async function selectSession(id) {
  state.selected=id;state.selectedRoutine=null;setRoute("session",id);renderFleet();renderRoutines();$("routine-view").classList.add("hidden");
  await loadDetail();
}
function selectedRoutine(){return state.routines.find(r=>r.routine_id===state.selectedRoutine)||null;}
function selectRoutine(id){state.selectedRoutine=id;state.selected=null;state.detail=null;setRoute("routine",id);renderFleet();renderRoutines();$("empty").classList.add("hidden");$("session-view").classList.add("hidden");$("inspector").classList.add("hidden");$("routine-view").classList.remove("hidden");renderRoutineDetail();}
...
async function loadDetail() {
  const sessionId=state.selected;if(!sessionId)return;
  try { const detail=await api(`/api/sessions/${encodeURIComponent(sessionId)}`);if(state.selected!==sessionId)return;state.detail=detail;renderDetail(); }
...
function renderDetail() {
  ...
  const plan=session.plan||summary.plan||[],completed=plan.filter(x=>x.status==="completed").length,goalItem=plan.find(x=>["in_progress","working"].includes(x.status))||plan.find(x=>x.status!=="completed"),activeGoal=(session.current_work||summary.current_work||{}).summary||goalItem?.content||goalItem?.title||"No active goal"; $("plan-count").textContent=`${completed}/${plan.length}`;
  $("current-work").textContent=activeGoal;
...
function populateConfig(config,known,source,pending) {
  state.originalConfig={...config};const sourceNote=source==="persisted_partial"?"Only recorded changes are shown; this legacy session has no trustworthy record for the other settings.":source==="session_metadata_partial"?"Reasonix session metadata contains only some settings; missing values are not guessed.":"Not reported by the current agent daemon";
  const pendingText=Object.entries(pending||{}).map(([k,v])=>`${k}: ${v}`).join(" · ");
  $("current-config").innerHTML=[['Model',config.model],['Effort',config.effort],['Approval',config.tool_approval],['Mode',config.work_mode]].map(([k,v])=>`<div><span>${k}</span><strong class="${!v?"unknown":!known?"unverified":""}">${escapeHtml(v||"Unknown")}</strong></div>`).join("")+(!known?`<p>${escapeHtml(sourceNote)}</p>`:"")+(pendingText?`<p class="pending-config">Queued: ${escapeHtml(pendingText)}</p>`:"");
...
function scheduleRefresh(event={}){const sessionId=event.session_id||(event.event||{}).session_id;if(sessionId)state.refreshSessions.add(sessionId);else state.refreshAll=true;if(state.refreshTimer)return;state.refreshTimer=setTimeout(flushRefresh,160);}
async function flushRefresh(){state.refreshTimer=null;if(state.refreshing){state.refreshTimer=setTimeout(flushRefresh,160);return;}const sessions=new Set(state.refreshSessions),refreshAll=state.refreshAll;state.refreshSessions.clear();state.refreshAll=false;state.refreshing=true;try{await refreshFleet();if(state.selected&&(refreshAll||sessions.has(state.selected)))await loadDetail();}catch(e){toast(e.message,true);}finally{state.refreshing=false;}if(state.refreshAll||state.refreshSessions.size)scheduleRefresh();}
...
function applyRoutineTemplate(){const t=state.templates.find(x=>x.template_id===$("routine-template").value);$("routine-template-description").textContent=t?.description||"Start blank or choose a curated maintenance workflow; every field remains editable.";if(!t)return;$("routine-name").value=t.name;$("routine-instructions").value=t.prompt;$("routine-schedule").value=t.schedule_kind;$("routine-daily").value=t.daily_at;$("routine-interval").value=t.interval_minutes;$("routine-timezone").value=t.timezone;$("routine-overlap").value=t.overlap_policy;$("routine-delegation").value=t.delegation;$("routine-approval").value=t.tool_approval;$("routine-mode").value=t.work_mode;$("routine-enabled").checked=t.enabled;setRoutineScheduleFields();}
function openRoutineForm(r=null){
  const owners=state.fleet?.orchestrators||[],models=state.fleet?.models?.models||[],efforts=state.fleet?.models?.effort_options||[];
  $("routine-form-title").textContent=r?"Edit loop agent":"New loop agent";$("routine-edit-id").value=r?.routine_id||"";
  $("routine-template-field").classList.toggle("hidden",Boolean(r));$("routine-template-description").classList.toggle("hidden",Boolean(r));
  $("routine-template").innerHTML=`<option value="">Blank / custom</option>`+state.templates.map(t=>`<option value="${escapeHtml(t.template_id)}">${escapeHtml(t.name)} · ${escapeHtml(t.category)}</option>`).join("");$("routine-template").value="";
  $("routine-name").value=r?.name||"";$("routine-owner").innerHTML=owners.map(o=>`<option value="${escapeHtml(o.owner_id)}">${escapeHtml(o.label)}</option>`).join("");$("routine-owner").value=r?.owner_id||owners[0]?.owner_id||"";$("routine-owner").disabled=Boolean(r);$("routine-cwd").value=r?.cwd||"";$("routine-instructions").value=r?.prompt||"";
  $("routine-schedule").value=r?.schedule_kind||"daily";$("routine-daily").value=r?.daily_at||"09:00";$("routine-interval").value=r?.interval_minutes||1440;$("routine-timezone").value=r?.timezone||Intl.DateTimeFormat().resolvedOptions().timeZone||"UTC";$("routine-overlap").value=r?.overlap_policy||"skip";$("routine-delegation").value=r?.delegation||"allowed";
  $("routine-model").innerHTML=models.map(m=>`<option value="${escapeHtml(m.ref)}">${escapeHtml(m.ref)}</option>`).join("");$("routine-model").value=r?.model||state.fleet?.models?.default||models[0]?.ref||"";$("routine-effort").innerHTML=efforts.map(v=>`<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`).join("");$("routine-effort").value=r?.effort||"max";$("routine-approval").value=r?.tool_approval||"auto";$("routine-mode").value=r?.work_mode||"balanced";$("routine-enabled").checked=r?.enabled??true;$("routine-immediate").checked=false;$("run-now-label").classList.toggle("hidden",Boolean(r));applyRoutineTemplate();setRoutineScheduleFields();$("routine-dialog").showModal();
}
function routineFormData(){return {template_id:$("routine-template").value,name:$("routine-name").value.trim(),owner_id:$("routine-owner").value,cwd:$("routine-cwd").value.trim(),prompt:$("routine-instructions").value.trim(),schedule_kind:$("routine-schedule").value,daily_at:$("routine-daily").value,interval_minutes:Number($("routine-interval").value),timezone:$("routine-timezone").value.trim(),overlap_policy:$("routine-overlap").value,delegation:$("routine-delegation").value,model:$("routine-model").value,effort:$("routine-effort").value,tool_approval:$("routine-approval").value,work_mode:$("routine-mode").value,enabled:$("routine-enabled").checked,run_immediately:$("routine-immediate").checked};}
...
async function authenticate(password){state.password=password;try{const route=routeFromLocation();await api("/api/health");sessionStorage.setItem("reasonix-dashboard-password",password);$("auth-screen").classList.add("hidden");$("app").classList.remove("hidden");await refreshFleet(false);if(route.routine&&state.routines.some(r=>r.routine_id===route.routine))selectRoutine(route.routine);else if(route.session)await selectSession(route.session);eventStream();}catch(e){state.password="";sessionStorage.removeItem("reasonix-dashboard-password");$("auth-screen").classList.remove("hidden");$("app").classList.add("hidden");toast("Invalid dashboard password",true);}}
...
$("new-routine").onclick=()=>openRoutineForm();$("routine-edit").onclick=()=>openRoutineForm(selectedRoutine());$("routine-template").onchange=applyRoutineTemplate;$("routine-schedule").onchange=setRoutineScheduleFields;for(const id of ["routine-cancel","routine-cancel-bottom"])$(id).onclick=()=>$("routine-dialog").close();
$("routine-form").onsubmit=async e=>{e.preventDefault();const id=$("routine-edit-id").value,data=routineFormData();try{let saved;if(id){delete data.owner_id;delete data.run_immediately;saved=await api(`/api/routines/${encodeURIComponent(id)}/configure`,{method:"POST",body:JSON.stringify(data)});}else saved=await api("/api/routines",{method:"POST",body:JSON.stringify(data)});$("routine-dialog").close();await refreshFleet();selectRoutine(saved.routine_id);toast(id?"Routine updated":"Routine created");}catch(err){toast(err.message,true);}};
$("routine-run").onclick=()=>routineAction("run");$("routine-stop").onclick=()=>{if(confirm("Stop the active routine parent agent?"))routineAction("stop");};$("routine-toggle").onclick=()=>{const r=selectedRoutine();routineAction("configure",{enabled:!r.enabled});};$("routine-delete").onclick=()=>{if(confirm("Delete this routine and its run history?"))routineAction("delete").then(()=>{state.selectedRoutine=null;setRoute();$("routine-view").classList.add("hidden");$("empty").classList.remove("hidden");});};
