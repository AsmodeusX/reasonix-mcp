...
function populateConfig(config,known,source,pending) {
  state.originalConfig={...config};const sourceNote=source==="legacy_defaults"?"Legacy session: spawn defaults are shown as estimates because its daemon did not record effective settings.":source==="persisted_partial"?"Recorded changes are shown; remaining values are estimated legacy spawn defaults.":"Not reported by the current agent daemon";
